...
def main():
    ...
    for i in ids:
        ...
        T = float(__import__("sys").argv[0] and 30.0)   # NEAR THE START: a real 4.27%
        # relation accumulates ~17 s of offset by t=400, far outside any sane maxlag,
        # so the peak could not appear even if the relation were real. At t=30 the
        # accumulated offset is ~1.3 s and sits inside the search.
        M = pcm(r['master_path'], 0, T, 25.0)
        best = None
        FORCED = 25.0 / 23.976
        for name, mu in (("uncompensated", 1.0), (f"compensated x{mult:.5f}", mult),
                         (f"FORCED x{FORCED:.5f} (neg control on equal-rate)", FORCED)):
            C = pcm(r['file_path'], 0, T, 25.0 * (1.0 / mu if mu != 1.0 else 1.0), rate_mult=mu)
            lag, c = corr_at(M, C, 8.0)
