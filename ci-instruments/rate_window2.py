...
for i in [int(x) for x in sys.argv[1:]]:
    r = recs[i]
    # FIRST find the coarse lag on a window big enough to contain it, THEN remove it
    # and vary the window. A 2 s window cannot search +-3.17 s -- the same
    # search-window-too-narrow error I have now made three times in this thread.
    M0 = pcm(r['master_path'], 0, 30.0, 25.0)
    C0 = pcm(r['file_path'], 0, 30.0, 25.0/MULT, rate_mult=MULT)
    base, bc = corr(M0, C0, 8.0)
    print(f"  id {i}: coarse lag {base:.1f} ms at corr {bc:.4f} -- removed below")
    for W in (2.0, 5.0, 10.0, 20.0, 25.0):
        M = pcm(r['master_path'], 0, 30.0, W)
        # shift the candidate read so the known offset is already gone
