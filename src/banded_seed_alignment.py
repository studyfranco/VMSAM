...
def b2_align(fp_master, fp_candidate, quantum_ms, band=None, k=K, bits=B,
    ...
    context:
      ...
      "all_zones"                EVERY zone between adjacent segments whose offsets differ, INCLUDING
                                  ones classified `below_resolution_floor` -- a measurement, once
                                  made, is never silently discarded (see `RESOLUTION_FLOOR_QUANTA`).
    ...
    for seg_a, seg_b in zip(segments, segments[1:]):
        ...
        zone = {
            ...
            "step_ms": step_ms,
            "classification": ("below_resolution_floor" if abs(step_ms) < floor_ms else "cut"),
