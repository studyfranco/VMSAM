...
def normalize_segments(segments, master_duration_ms, candidate_duration_ms,
    ...
    # reference.
    offsets = set(Decimal(str(segment["candidate_offset_ms"])) for segment in ordered)
...
def choose_probe_positions(pieces, window_seconds):
    ...
    # ne voit pas.
    candidates = [p for p in pieces if p["source"] == "candidate"
                  and (p["master_end_ms"] - p["master_start_ms"]) >= window_ms]
    candidates.sort(key=lambda p: p["master_end_ms"] - p["master_start_ms"], reverse=True)
    positions = []
    if len(candidates) == 1:
        # Une tranche unique, c'est le cas d'une relation de VITESSE seule. Un
        # seul sondage au milieu y verrait mal: une erreur de coefficient
        # residuelle e produit un decalage e x t, donc nul au debut et maximal
        # aux extremites. On etale les sondages pour que la derive ait ou se
        # montrer, au lieu de la mesurer la ou elle est la plus petite.
        #
        # Les sondages VONT JUSQU'AUX DEUX BORDS, pas de 1/8 a 7/8. vmsam-dev-1 a
        # mesure que la geometrie du pipeline rend les ~227 premieres secondes
        # d'un episode invisibles a son balayage: un pas s'y cache, et sur un plan
        # a tranche unique il n'y a rien d'autre pour le voir. Une sonde qui
        # commence a 1/8 laisserait les 178 premieres secondes sans controle,
        # c'est-a-dire exactement la ou l'angle mort de la mesure se trouve.
        piece = candidates[0]
        span = piece["master_end_ms"] - piece["master_start_ms"] - window_ms
        if span < 0:
            span = Decimal("0")
        divisor = Decimal(max(1, verify_max_probes - 1))
        for i in range(verify_max_probes):
            positions.append(piece["master_start_ms"] + span * Decimal(i) / divisor)
    else:
        ...
        chosen = [candidates[0]] if len(candidates) else []
        by_position = sorted(candidates, key=lambda p: p["master_start_ms"])
        for piece in (by_position[0], by_position[-1]):
        ...
        for piece in candidates:
            ...
            if piece not in chosen:
                chosen.append(piece)
        for piece in chosen:
            middle = (piece["master_start_ms"] + piece["master_end_ms"]) / 2
            start = middle - window_ms / 2
            # rester dans la tranche: sur une tranche courte le milieu moins une
            # demi-fenetre peut deborder en amont
            if start < piece["master_start_ms"]:
                start = piece["master_start_ms"]
            if start + window_ms > piece["master_end_ms"]:
                start = piece["master_end_ms"] - window_ms
...
def verify_on_master_timeline(out_path, master_obj, audio_reports, pieces,
    ...
    """
    positions = choose_probe_positions(pieces, verify_window_seconds)
    ...
    for report in audio_reports:
        ...
        for start in positions:
            ...
            probes.append({"master_position_ms": str(start), "lag_ms": lag,
