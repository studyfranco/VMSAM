...
class video():
    ...
    def extract_audio_in_part(self,language,exportParam,cutTime=None,asDefault=False):
        if (not self.lastCutAsDefault) or (not asDefault):
            ...
            baseCommand = [tools.software["ffmpeg"], "-y", "-probesize", "50000000", "-threads", str(3), "-nostdin", "-i", self.filePath, "-vn"]
            ...
            if cutTime == None:
                for audio in self.audios[language]:
                    if audio["compatible"]:
                        ...
                        self.ffmpeg_progress_audio.append(ffmpeg_pool_audio_convert.apply_async(tools.launch_cmdExt_with_timeout_reload, (cmd,3,1200)))
            else:
                for audio in self.audios[language]:
                    if audio["compatible"]:
                        nameFilesExtractCut = []
